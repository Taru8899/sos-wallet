...
contract PresenceLogger {
     ...
     * @return True if the Presence exists and is outside the live window
     *         (eligible for erasure), false otherwise.
     */
    function isExpired(uint256 id) public view returns (bool) {
        require(id > 0 && id < nextId, "Invalid id");
        Presence storage p = presences[id];
        if (p.timestamp == 0) return false; // already erased
        return id + MAX_LIVE_PRESENCES < nextId;
    }
    ...
    function getPresence(uint256 id) external view returns (Presence memory) {
        require(id > 0 && id < nextId, "Invalid id");
        Presence memory p = presences[id];
     ...
     * @return Total count of Presence ids issued so far.
     */
